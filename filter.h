...
static image<rgb> *smooth_rgb(image<rgb> *input, float sigma) {
...
int width = input->width();
  ...
  image<float> *smooth_b = smooth(b, sigma);
   for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      imRef(smoothed, x, y).r = imRef(smooth_r, x, y) ;
      imRef(smoothed, x, y).g = imRef(smooth_g, x, y) ;
      imRef(smoothed, x, y).b = imRef(smooth_b, x, y) ;
    }
  }
  return smoothed;
}
